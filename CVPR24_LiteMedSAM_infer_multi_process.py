...
# medsam_lite_model = efficientvit_sam_l0()
...
def medsam_inference(medsam_model, img_embed, box_256, new_size, original_size):
    ...
    # medsam_seg = (low_res_pred > 0.5).to(torch.uint8)

    return medsam_seg, iou


def MedSAM_infer_npz_2D(img_npz_file, new_size =512):
    ...
    img_256_padded = pad_image(img_256_norm, new_size)
    ...
    # for idx, box in enumerate(boxes, start=1):
        box256 = resize_box_to_256(boxes, original_size=(H, W),new_size = new_size)
        ...
        # for i in range(medsam_mask.shape[0]):
        #     segs[medsam_mask[i]>0] = i+1
            # print(f'{npz_name}, box: {box256[i]}, predicted iou: {np.round(iou_pred.cpu().numpy()[i], 4)}')
