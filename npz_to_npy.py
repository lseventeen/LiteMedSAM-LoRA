...
parser.add_argument("-npz_dir", type=str, default="/ai/data/data/cvpr24-medsam/train_npz",
                    help="Path to the directory containing preprocessed .npz data, [default: data/npz/MedSAM_train/CT_Abd]")
parser.add_argument("-npy_dir", type=str, default="/ai/data/data/cvpr24-medsam/data_npy_all",
...
for root, dirs, files in os.walk(npz_dir):
    ...
    for file in files:
        ...
        if file.endswith(".npz"):
            # If it does, append the file name to npz_names
            npz_names.append(os.path.join(root, file))
...
def convert_npz_to_npy(npz_name):
    ...
    """
    name = npz_name.split(".npz")[0].split("/")[-1]
